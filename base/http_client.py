...
class PycurlClient(HTTPClient):
    ...
    def parse_headers(self, data):
        ...
        return dict((k.title(), v) for k, v in dict(headers).iteritems())

    def request(self, method, url, headers, post_data=None, **kwargs):
        import pycurl

        s = StringIO.StringIO()
        rheaders = StringIO.StringIO()
        curl = pycurl.Curl()
        ...
        self.content, self.code, self.headers = rbody, rcode, self.parse_headers(rheaders.getvalue())
...
class Urllib2Client(HTTPClient):
    ...
    def request(self, method, url, headers, post_data=None, handlers=[], **kwargs):
        ...
        try:
            ...
            rbody = response.read()
            rcode = response.code
            headers = dict((k.title(),v) for k,v in dict(response.info()).iteritems())
        except urllib2.HTTPError, e:
            log.info('response from %s', e.fp.fp._sock.fp._sock.getpeername())
            rbody = e.read()
            rcode = e.code
            headers = dict((k.title(),v) for k,v in dict(e.info()).iteritems())
